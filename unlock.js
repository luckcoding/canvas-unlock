...
function GesturesUnlock() {
    ...
    this.arcLineWidth = 5; // 圆的粗细
    this.touchColor = '#FF8822'; // 手指触摸的线条颜色
    ...
    // start the mini view modal
    var getLockLite = new GetLockLite();
    getLockLite.init();
    ...
    this.init = function (callBack) {
        ...
        if (canvas.getContext) {
            ...
            // 9个点的全部坐标
            var pointArr = ninePoint('gesturesUnlock');
            
            // 绘制9个点位圆
            ninePointArc('gesturesUnlock',ctx,this.radius,this.arcColor,this.arcLineWidth);
            ...
            canvas.addEventListener('touchstart', function (e) {
                ...
                for (key in pointArr) {
                    if (touchX > (pointArr[key].x - thisObj.radius) && touchX < (pointArr[key].x + thisObj.radius) && touchY > (pointArr[key].y - thisObj.radius) && touchY < (pointArr[key].y + thisObj.radius)) {
                        ...
                        pointArc(ctx, pointArr[key].x, pointArr[key].y, thisObj.radius, thisObj.arcDidColor, thisObj.arcLineWidth);
                        // 把密码值加入密码结果
                ...
                canvas.addEventListener('touchend', function () {
                    if (thisObj.pwdResult) {
                        // render the mini view modal
                        getLockLite.render(thisObj.pwdResult)
...
GesturesUnlock.prototype.reNewLine = function (ctx) {
    ...
    canvas.addEventListener('touchmove', function (eve) {
        ...
        // 绘制刚被选中的圆
        pointArc(ctx, thisObj._tempPoint.x, thisObj._tempPoint.y, thisObj.radius, thisObj.arcDidColor, thisObj.arcLineWidth);
        // 如果临时绘制数据存在值则把他放到画布上
...
function GetLockLite (callBack) {
    ...
    this.arcDidColor = '#FF8822'; // 选中圆的颜色
    this.init = function () {
        var thisObj = this;
        var canvas = document.getElementById('getLockLite');
        // 获取画布宽高
        var _guaWidth = canvas.width;
        var _guaHeight = canvas.height;
        if (canvas.getContext) {
            ...
            ctx.clearRect(0, 0, _guaWidth, _guaHeight);
            // 9个点的全部坐标
            var pointArr = ninePoint('getLockLite');
            // 绘制9个点位圆
            ninePointArc('getLockLite', ctx, this.radius, this.arcColor, this.arcLineWidth);
            // thisObj.render(ctx,pwd)
        }
    }
    this.render = function (pwd) {
        var canvas = document.getElementById('getLockLite');
        var ctx = canvas.getContext('2d');
        var pointArr = ninePoint('getLockLite');
        for (var i = 1; i <= pwd.length; i++) {
            var key = pwd.slice(i-1, i) - 1
            pointArc(ctx, pointArr[key].x, pointArr[key].y, this.radius, this.arcDidColor, this.arcLineWidth)
        }
    }
...
function ninePointArc (domId,ctx,radius,arcColor,arcLineWidth) {
    ...
    for (key in pointArr) {
        // 画大空心圆
        pointArc(ctx, pointArr[key].x, pointArr[key].y, radius, arcColor, arcLineWidth);
    }
}